...
@with_kw struct GeochemicalDomainDistribution
    N = 32
    center = product_distribution([Uniform(N/4., 3N/4.), Uniform(N/4., 3N/4.)])
    points = [product_distribution([Uniform(-N/3.5, N/3.5), Uniform(-N/3.5, N/3.5)]) for i=1:25]
    angle = Uniform(0, 2π)
...
"""
Base.rand(rng::AbstractRNG, b::GeochemicalDomainDistribution) = (rand(rng, b.center), [rand(rng, p) for p in b.points], rand(rng, b.angle))

"""
Samples a geochemical domain (blob shape) for the provided parameters. 
"""
function draw_geochemical_domain(N, center, pts, angle; σ=N/25, grayscale=0.4, threshold=0.17)
...
@with_kw struct GrabenDistribution
    N = 32
    left_top = Uniform(0.15 * N, N)
    left_width = Uniform(0.05 * N, 0.25 * N)
    right_top = Uniform(0.15 * N, N)
    right_width = Uniform(0.05 * N, 0.25 * N)
    μ           # Mean of the GP within this domain
end
...
"""
Base.rand(rng::AbstractRNG, b::GrabenDistribution) = (rand(rng, b.left_top), rand(rng, b.left_width), rand(rng, b.right_top), rand(rng, b.right_width))
...
function draw_graben(N, ltop, lwidth, rtop, rwidth)
    ...
    v = VPolygon([
        [1.0, ltop], [1.0, ltop - lwidth], [N, rtop - rwidth], [N, rtop]
    ])
