...
function TwoGeochemDomain()
    return GeochemicalDomainGenerator(;
        grades=[5.0, 7.0], locations=[[25, 25], [55, 55]], radii=[15.0, 15.0], Npts=10
    )
end
function OneGeochemDomain()
    return GeochemicalDomainGenerator(;
        grades=[7.0], locations=[[35, 35]], radii=[20.0, 20.0], Npts=10
    )
end
...
function Base.rand(g::GrabenDomainGenerator)
    ...
    for i in 1:(g.Ngrabens)
        offset = g.Ng[1] * (i - 1) / g.Ngrabens
        ltop, lwidth = offset + (1.0 / g.Ngrabens) * rand(g.left_top_dist),
        rand(g.left_width_dist)
        rtop, rwidth = offset + (1.0 / g.Ngrabens) * rand(g.right_top_dist),
        rand(g.right_width_dist)

        # Compute the polygon representing the Graben
        v = VPolygon([
            [1.0, ltop], [1.0, ltop - lwidth], [g.Ng[1], rtop - rwidth], [g.Ng[2], rtop]
        ])
...
function compute_logpdf(
    domain_ensemble, xobs, thickness_obs, grade_obs, structural_obs, geochem_obs
)
    logpdfs_structural = []
    logpdfs_geochem = []
    ...
    return logsumexp(logpdfs_structural) +
           log(1.0 / length(logpdfs_structural)) +
           logsumexp(logpdfs_geochem) +
           log(1.0 / length(logpdfs_geochem))
end

function compute_null_pdf(
    thickness_dist,
    thickness_obs,
    grade_dist,
    grade_obs,
...
function plot_domain(domain, xobs=[], structural_obs=[], geochem_obs=[])
    ...
    return plot(ps, pg; colorbar=false, size=(800, 400))
end

function plot_mineralization(geology)
    return heatmap(
        geology[:, :, 1]' .* geology[:, :, 2]' ./ 40.0;
...
hypotheses = [
    GeologicalDomainGenerator(g, c) for g in graben_hypotheses, c in geochem_hypotheses
][:]
ensembles = [[rand(h) for _ in 1:10000] for h in hypotheses]
...
histogram!(
    s[:, :, 1][:]; bins=0:1:15, normalize=true, alpha=0.5, label="Model", size=(400, 400)
)
...
structural_obs =
    domain_gt[:, :, 1][:][r_indices] .+
    rand(Normal(0.0, DOMAIN_OBS_NOISE), length(r_indices))
geochem_obs =
    domain_gt[:, :, 2][:][r_indices] .+
    rand(Normal(0.0, DOMAIN_OBS_NOISE), length(r_indices))
...
plots = []
anim = @animate for i in 1:length(logpdfs_1)
    p1 = plot(
        1:i,
        logpdfs_2[1:i];
        label="H1",
        xlims=(0, length(logpdfs_1)),
        ylabel="Logpdf",
        legend=:bottomleft,
        yformatter=y_formatted,
        title="Logpdfs of Hypotheses",
        margin=5Plots.mm,
    )
